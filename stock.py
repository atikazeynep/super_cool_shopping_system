...
def main():
    ...
    while True:
        ...
        if stock[product_id] == 0:
            print(f"{products[product_id]['name']} is out of stock. Please choose another product.")
            continue
        ...
        try:
            ...
            if quantity > stock[product_id]:
                print(f"Only {stock[product_id]} units of {products[product_id]['name']} are available. Please enter a lower quantity.")
                continue
        except ValueError:
